document.addEventListener('DOMContentLoaded', () => {
  ...
  const projects = [
    ...
    },
    {
      ...
      "date": "Juin 2026",
      "stack": "Full-stack • Architecture • GitHub",
  ...
  if (projectsContainer) {
    projectsContainer.className = "projects-slider-viewport";

    projectsContainer.innerHTML = projects.map((project, index) => `
      <div class="project-card-3d" data-index="${index}">
        <div style="width: 220px; height: 130px; flex-shrink: 0; overflow: hidden; border-radius: 12px; background: rgba(255,255,255,0.02);">
          <img src="${project.image}" alt="${project.titre}" style="width: 100%; height: 100%; object-fit: cover;">
        </div>
        <div style="flex-grow: 1; display: flex; flex-direction: column; justify-content: center;">
          <div style="display: flex; gap: 1rem; align-items: center; margin-bottom: 0.4rem;">
            <span class="bento-tag" style="margin-bottom: 0;">${project.date}</span>
            <span style="color: #72c050; font-size: 0.8rem; font-weight: 600;">${project.stack}</span>
          </div>
          <h2 style="margin: 0 0 0.4rem 0; font-size: 1.25rem; color: #ffffff;">${project.titre}</h2>
          <p class="bento-preview" style="margin: 0; font-size: 0.9rem;">${project.descriptionCourte}</p>
        </div>
      </div>
      ...
      <div id="${project.id}" class="modal">
        <div class="modal-content">
          ...
          <h2>${project.titre}</h2>
          <p style="color: #72c050; font-weight: 600; margin-bottom: 1rem;">${project.date} — ${project.stack}</p>
          <div style="width: 100%; max-height: 280px; overflow: hidden; border-radius: 12px; margin-bottom: 1.25rem; background: rgba(0,0,0,0.2);">
            <img src="${project.image}" alt="${project.titre}" style="width: 100%; height: 100%; object-fit: contain;">
          </div>
          <p style="line-height: 1.6; color: #cbd5e1;">${project.descriptionLongue}</p>
        </div>
      </div>
    `).join('');

    let activeProjIndex = 0;
    const projCards = projectsContainer.querySelectorAll('.project-card-3d');
    
    projCards.forEach(card => {
      card.addEventListener('click', () => {
        const idx = parseInt(card.getAttribute('data-index'), 10);
        if (idx !== activeProjIndex) {
          activeProjIndex = idx;
          updateProjCarousel();
        } else {
          const modalId = projects[activeProjIndex].id;
          const modal = document.getElementById(modalId);
    ...
    function updateProjCarousel() {
      projCards.forEach((card) => {
        const cardIndex = parseInt(card.getAttribute('data-index'), 10);
        card.classList.remove('state-center', 'state-top', 'state-bottom', 'state-hidden');

        if (cardIndex === activeProjIndex) {
          card.classList.add('state-center');
        } else if (cardIndex === (activeProjIndex - 1 + projCards.length) % projCards.length) {
          card.classList.add('state-top');
        } else if (cardIndex === (activeProjIndex + 1) % projCards.length) {
          card.classList.add('state-bottom');
        } else {
